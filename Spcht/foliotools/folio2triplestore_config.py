...
# ? this is most stupid, let me tell you the story of this real quick:
# ? this was a side project, i wanted quick results and wrote some things together that worked within in the scope
# ? of what i wanted. For some reason i thought i make the config file this time a python one so i can access the
# ? content directly without nestling around with schemes or the like. Later i created this thing as a package which
# ? makes editing a part of that package a bit unwieldy. The band aid fix is to use a crowbar to create dynamic
# ? attributes within a module dynamically read from a json on module import. I guess i am now on the most wanted list
# ? of some python gurus but this works. Problematic is..as this is part of the package it gets loaded every time, even
# ? when you dont access any part of it. And that is where i made the condition below, so it only triggers when using
# ? the hardcoded part of the package that actually does use it. Its kinda neat but also totally cowboyish
if Path(__main__.__file__).name == "folio2triplestore.py":
