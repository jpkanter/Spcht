    ...
    try:
        ...
        for n, elem in enumerate(doc):
            if isinstance(elem, dict):
                ...
                if (not text or elem.get("@context") == text) and elem.get("@context") and isinstance(
                        elem.get("@context"), str):
                ...
                if isinstance(elem.get("about"), list):
                    for m, rvkl in enumerate(elem.get("about")):
                        if isinstance(rvkl.get("identifier"), dict) and rvkl.get("identifier").get(
...
if __name__ == "__main__":
    # parser library for cli helper und input
    ...
    parser.add_argument('-scroll', action="store_true",
                        help="print out the whole index as RDF instead getting a single doc")
    parser.add_argument('-server', type=str,
                        help="use http://host:port/index/type/id?pretty syntax. overwrites host/port/index/id")
    parser.add_argument('-context', type=str,
                        help="deliver a url to the context if there is no @context field in the data")
    ...
    parser.add_argument('-format', type=str, default="nt",
                        help="rdflib format to use for output. default=nt (N-Triple), allowed values: nquads, turtle, application/rdf+xml")
